...
const Add = ({token}) => {
  ...
  const [category, setCategory] = useState("Aquamarine");
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      {/* Flex container for Category, Color, and Price */}
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 w-full max-w-[500px]">
        <div className="flex-1">
          <p className='mb-2'>Category</p>
          <select onChange={(e)=>setCategory(e.target.value)} value={category} className='w-full px-3 py-2'>
            <option value="Rutilated Quartz">Rutilated Quartz</option>
            <option value="Phantom Quartz">Phantom Quartz</option>
        ...
        <div className="flex-1">
          ...
          <input onChange={(e)=>setPrice(e.target.value)} value={price} className='w-full px-3 py-2' type="number" placeholder='888' />
        </div>
      </div>
